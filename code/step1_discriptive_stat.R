...
p2 <- ggplot(summary_stats_wd, aes(x = Year, y = avg_distance_km, color = AgeGroup, fill = AgeGroup)) +
  ...
  scale_x_continuous(breaks = seq(2008, 2024, by = 4)) +
  labs(
    y = "Mean total daily trip distance (km)",
    x = "Year"
  ) +
  theme_minimal(base_size = 14) +
  theme(
...
p3 <- ggplot(summary_stats_wd, aes(x = Year, y = avg_hours_away, color = AgeGroup, fill = AgeGroup)) +
  ...
  scale_x_continuous(breaks = seq(2008, 2024, by = 4)) +
  labs(
    y = "Mean first-to-last trip span (h)",
    x = "Year",
    color = "Age group",
    fill = "Age group"
  ) +
...
# Supplementary Figure S7, placed at \textwidth (6.5 in) in the SI. Canvas
# sized so base_size 14 lands near 7 pt on the page. See R2 comment 2.6.
ggsave(overleaf_fig_path, combined_plot, width = 10.4, height = 3.6, dpi = 400)
ggsave(paste0(output_dir, "Figure_R2_S7.png"), combined_plot, width = 10.4, height = 3.6, dpi = 400)
cat("\n✓ Supplementary Figure S7 saved to:\n  ", overleaf_fig_path, "\n")


# -------------------------------------------------------------------
# 10. LaTeX table BODIES for the manuscript
# -------------------------------------------------------------------
# Table 1 and Supplementary Table S1 were typed into the manuscript by
# hand. The manifest marked them RESTRICTED and passed them without
# looking, which is the same blind spot that let Supplementary Tables S6
# and S7 stay pre-fixed-weight for a full revision cycle. Both are now
# emitted as tabular bodies that the documents \input, so a number can
# only be wrong here if this script computes it wrong.
#
# Table 1 needs an all-days block as well as the weekday block that the
# rest of this script builds, so that sample is derived here from the same
# source tables. The filter that reproduces the published all-days block is
# DiaryYear >= 2007 and age >= 10, with no daytype or month restriction.
cat("\n=== GENERATING TABLE 1 AND TABLE S1 BODIES ===\n")

tables_dir <- file.path("results", "tables")
if (!dir.exists(tables_dir)) dir.create(tables_dir, recursive = TRUE)
manuscript_tables <- get_manuscript_table_dir()

publish_table <- function(lines, file) {
  path <- file.path(tables_dir, file)
  writeLines(lines, path)
  dest <- file.path(manuscript_tables, file)
  if (!file.copy(path, dest, overwrite = TRUE)) stop("Failed to write ", dest)
  cat("  ok", file, "->", dest, "\n")
}

# Three significant figures, the convention the published table uses for
# clock times.
sig3 <- function(x) formatC(signif(x, 3), format = "fg", digits = 3, flag = "#")

summarise_block <- function(sess) {
  sess <- sess %>%
    mutate(AgeGroup = factor(case_when(
      ...
      RespAgeCorrect <= 65 ~ "56-65",
      TRUE                 ~ "66+"),
      levels = c("10-17", "18-30", "31-55", "56-65", "66+"), ordered = TRUE))

  trips <- tur %>%
    semi_join(sess, by = "SessionId") %>%
    left_join(sess %>% dplyr::select(SessionId, AgeGroup), by = "SessionId") %>%
    filter(!is.na(DepartMSM), !is.na(ArrivalMSM), ArrivalMSM > DepartMSM) %>%
    group_by(SessionId, AgeGroup) %>%
    summarise(AvgDist   = sum(GISdist, na.rm = TRUE),
              FirstTrip = min(DepartMSM, na.rm = TRUE),
              LastTrip  = max(ArrivalMSM, na.rm = TRUE),
              .groups = "drop") %>%
    mutate(HoursAway = (LastTrip - FirstTrip) / 60)

  none <- sess %>%
    anti_join(trips %>% distinct(SessionId), by = "SessionId") %>%
    transmute(SessionId, AgeGroup, AvgDist = 0,
              FirstTrip = NA_real_, LastTrip = NA_real_, HoursAway = 0)

  bind_rows(trips %>% dplyr::select(SessionId, AgeGroup, AvgDist,
                                    FirstTrip, LastTrip, HoursAway), none) %>%
    group_by(AgeGroup) %>%
    summarise(n = n(),
              share_no_trips   = 100 * mean(is.na(FirstTrip) | HoursAway == 0),
              avg_distance_km  = mean(AvgDist, na.rm = TRUE),
              avg_hours_away   = mean(HoursAway, na.rm = TRUE),
              avg_first_trip_h = mean(FirstTrip, na.rm = TRUE) / 60,
              avg_last_trip_h  = mean(LastTrip,  na.rm = TRUE) / 60,
...
session_base <- session %>%
  filter(DiaryYear >= 2007, RespAgeCorrect >= 10) %>%
  dplyr::select(SessionId, RespAgeCorrect, DiaryYear, DiaryDaytype, DiaryMonth)

block_all <- summarise_block(session_base)
block_wd  <- summarise_block(session_base %>%
                               filter(DiaryDaytype %in% c(11, 12), DiaryMonth != 7))

block_rows <- function(b) {
  sprintf("%s & %s & %.1f & %.1f & %.2f & %s & %s \\\\",
          gsub("-", "--", as.character(b$AgeGroup), fixed = TRUE),
          format(b$n, big.mark = ",", trim = TRUE),
          b$share_no_trips, b$avg_distance_km, b$avg_hours_away,
          sig3(b$avg_first_trip_h), sig3(b$avg_last_trip_h))
}
...
publish_table(t1, "table_main1_descriptive.tex")

# Supplementary Table S1: the same quantities by age group and 3-year bin,
# with intervals. Two-line headers via \hdr, defined in the SI preamble.
s1 <- c(
  "\\begin{tabular}{l l r c c c c c}",
  "\\toprule",
  paste("\\textbf{Age group} & \\textbf{Year bin} & \\textbf{n} &",
        "\\hdr{Share 0 trips}{(\\%) [95\\% CI]} &",
        "\\hdr{Avg. dist.}{(km) [95\\% CI]} &",
        "\\hdr{Avg. first--last}{span (h) [95\\% CI]} &",
        "\\hdr{First trip}{(h) [95\\% CI]} &",
        "\\hdr{Last trip}{(h) [95\\% CI]} \\\\"),
  "\\midrule",
  vapply(seq_len(nrow(summary_stats_3yr)), function(i) {
    row <- summary_stats_3yr[i, ]
    paste0(
      gsub("-", "--", as.character(row$AgeGroup), fixed = TRUE), " & ",
      gsub("–", "--", row$YearBin), " & ",
      format(row$n, big.mark = ",", trim = TRUE), " & ",
      sprintf("%.1f (%s)", row$share_no_trips * 100, row$ci_no_trips_label), " & ",
      sprintf("%.1f (%s)", row$avg_distance_km, row$ci_distance_label), " & ",
      sprintf("%.2f (%.2f--%.2f)", row$avg_hours_away, row$ci_hours_low, row$ci_hours_high), " & ",
      sprintf("%.2f (%.2f--%.2f)", row$avg_first_trip_h, row$ci_first_low, row$ci_first_high), " & ",
      sprintf("%.2f (%.2f--%.2f)", row$avg_last_trip_h, row$ci_last_low, row$ci_last_high), " \\\\")
  }, character(1)),
...
publish_table(s1, "table_s1_descriptive.tex")
