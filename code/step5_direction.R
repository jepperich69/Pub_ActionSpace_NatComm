...
all_volume_metrics <- all_volume_metrics %>%
  mutate(
    ...
    Period = case_when(
      # NOTE: "2007-2009" is the real first survey period and must NOT be folded
      # into the baseline aggregate; BASELINE_LABEL only embeds those years so the
      # chain sorts correctly. Folding them averaged the two into a synthetic start.
      Period %in% c("Baseline", BASELINE_LABEL) ~ BASELINE_LABEL,
      TRUE ~ Period
    )
  )
