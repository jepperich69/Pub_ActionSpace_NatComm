...
# Scenario sample restrictions. These MUST be applied to the trip-making and
# the zero-trip sample alike: the two are concatenated into one analysis file,
# so a restriction applied to only one of them silently mixes populations.
# Until 2026-09-08 `HomeAdrCitySize > 100000` was live in the zero-trip block
# and commented out in the trip block, which paired a national sample of travel
# days with a big-city-only sample of stay-home days, and the share of
# stay-home days retained rose from 23% to 40% across the study window.
# Set them here, once, and let both filters read them.
if (!exists("CITY_MIN")) CITY_MIN <- NA_real_   # NA = no settlement-size restriction
if (!exists("SEX_ONLY")) SEX_ONLY <- NA_integer_ # NA = both sexes; 1 = male, 2 = female

# Purp19 code 1 = "Eget hjem" / "Home", per the enumvalues table in TU0624v1
# itself and DTU's published variable documentation for OrigPurp. session's
# DayStartPurp records where the diary day began and equals the first trip's
# OrigPurp in 100% of trip-making days, but is defined for no-trip days too --
# and 11.8% of those did NOT start at home. Testing both halves of the sample
# on the same variable keeps a day that was away all day from entering as a
# day spent entirely at r = 0.
HOME_PURP <- 1L
dt <- 24 / N_T
dd <- R_MAX / N_D
...
tur_enriched <- tur %>%
  ...
  filter(
    ...
    is.na(CITY_MIN) | HomeAdrCitySize > CITY_MIN,
    is.na(SEX_ONLY) | RespSex == SEX_ONLY,
    DayStartPurp == HOME_PURP,      # day begins at home; see note at HOME_PURP
  ...
  dplyr::select(SessionId, TurId, TurNr, DepartMSM, ArrivalMSM, DestDwelTime,
                GISdist, GISdistJourneyStartP, OrigPurp,
...
# The home-start test is applied in both filters above -- to the trip-making
# and the zero-trip sample alike -- so no separate pass is needed here.
...
zero_trip_obs <- session %>%
  filter(!(SessionId %in% original_trippers$SessionId),  # Never had trips originally
         ...
         is.na(SEX_ONLY) | RespSex == SEX_ONLY,
         is.na(CITY_MIN) | HomeAdrCitySize > CITY_MIN,
         DayStartPurp == HOME_PURP,   # 11.8% of no-trip days were away all day
         RespAgeCorrect >= 10) %>%
  mutate(
    Year = DiaryYear,
    Age  = RespAgeCorrect,
    ...
    GISdist = 0, GISdistJourneyStartP = 0, OrigPurp = 1L
  ) %>%
  dplyr::select(SessionId, TurId, TurNr, DepartMSM, ArrivalMSM, DestDwelTime,
                GISdist, GISdistJourneyStartP, OrigPurp, AgeGroup, Age,
                Cars, Male, Year, City, SessionWeight)

tur_enriched <- bind_rows(tur_enriched, zero_trip_obs) %>% arrange(SessionId, TurId, TurNr)

# --- Drop sessions with no usable expansion weight ---
# Eq. (2) previously mapped these to weight 0 while the unweighted kde2d still
# counted them at full strength, so the home share and the away density were
# estimated on different samples. Both are weighted now, so remove them once,
# explicitly, rather than letting a zero weight mean two different things.
bad_w <- tur_enriched %>%
  filter(is.na(SessionWeight) | !is.finite(SessionWeight) | SessionWeight <= 0) %>%
  pull(SessionId) %>% unique()
tur_enriched <- tur_enriched %>% filter(!SessionId %in% bad_w)
cat("Removed", length(bad_w), "sessions with missing/non-positive SessionWeight
")
...
# Departures and arrivals are each de-duplicated within themselves above, but
# not against each other. When a departure and an arrival fall in the same
# 15-min bin -- a short trip, or a short dwell between two trips -- the joined
# grid row was duplicated, so the "N = 96" day of the Methods was not unique:
# 61,986 duplicate session-time rows, 33,661 of them carrying conflicting r.
# The later event is the person's state at the end of the bin, and is correct
# either way round: an arrival gives where they ended up, a departure gives
# where they were, and those coincide.
all_location_events <- bind_rows(trips_dep, trips_arr_with_dist) %>%
  group_by(SessionId, TimeBin) %>%
  slice_max(ev_time, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  dplyr::select(-ev_time) %>%
...
# Weighted product-Gaussian KDE -- Eq. (3) of the manuscript.
#
# MASS::kde2d cannot take observation weights: it hard-codes 1/nx, so the
# survey expansion weights never reached the away density even though Eq. (3)
# carries w_i inside the kernel sum. Eq. (2) (the home share) was weighted, so
# the two halves of the same decomposition used different populations.
# SessionWeight runs 1 to 4,213 (CV 0.70), so this is not a rounding matter.
#
# This is MASS's own algorithm with w_i in place of 1/nx, accumulated in
# chunks so cost is linear in n and memory stays flat (~15 MB at chunk=20000).
# That removes the need for the old MAX_PER_GROUP subsample, which applied a
# weighted resample to the two largest cells and left the other 28 unweighted
# -- a regime change inside the 31-55 group whose periods are differenced.
#
# Reduces to MASS::kde2d exactly when all weights are equal; see the
# equal-weight assertion below.
kde2d_w <- function(x, y, w, h, n, lims, chunk = 20000L) {
  gx <- seq.int(lims[1], lims[2], length.out = n[1])
  gy <- seq.int(lims[3], lims[4], length.out = n[2])
  h  <- h / 4
  w  <- w / sum(w)
  z  <- matrix(0, n[1], n[2])
  for (k in split(seq_along(x), ceiling(seq_along(x) / chunk))) {
    Kx <- matrix(dnorm(outer(gx, x[k], "-") / h[1]), n[1])
    Ky <- matrix(dnorm(outer(gy, y[k], "-") / h[2]), n[2])
    z  <- z + tcrossprod(Kx * rep(w[k], each = n[1]), Ky)
  }
  list(x = gx, y = gy, z = z / (h[1] * h[2]))
}

local({
  set.seed(1); x <- rnorm(500); y <- rnorm(500)
  a <- MASS::kde2d(x, y, n = c(24, 24), lims = c(-3, 3, -3, 3), h = c(1, 1))$z
  b <- kde2d_w(x, y, rep(1, 500), h = c(1, 1), n = c(24, 24), lims = c(-3, 3, -3, 3))$z
  stopifnot(max(abs(a - b)) < 1e-10)
  cat("kde2d_w: equal-weight identity with MASS::kde2d verified
")
})
  ...
  data_away <- data %>% filter(r_rad_km > home_zero, SessionWeight > 0)
  ...
  if (n_away >= 50) {
    kde_away <- with(data_away, kde2d_w(
      x = TimeMSM / 60, y = r_rad_km, w = SessionWeight,
      n = c(n_t, n_d),
      lims = c(0, 24, 0, r_max),
      h = c(h_t, h_d)
    ))
...
# 2. Spot-check one diary's time away over the 07:30-15:00 window. No diary id is
#    hardcoded: set ACTIONSPACE_DEBUG_SESSION to inspect a specific one, otherwise
#    the first session in the grid is used.
dbg_env  <- Sys.getenv("ACTIONSPACE_DEBUG_SESSION", "")
dbg_sess <- if (nzchar(dbg_env)) as.integer(dbg_env) else dplyr::first(grid_mv$SessionId)

check_dbg <- grid_mv %>%
  filter(SessionId == dbg_sess, TimeMSM >= 450, TimeMSM <= 900) %>%
  dplyr::select(SessionId, TimeMSM, r_rad_km, d_cum_km) %>%
  mutate(is_away = as.integer(r_rad_km > 0.02))

cat("\nSession", dbg_sess, "time away:\n")
cat("  Total away minutes:", sum(check_dbg$is_away) * 15, "\n")
cat("  Total away hours:", sum(check_dbg$is_away) * 15 / 60, "\n")

print(check_dbg, n = 31)
