...
# ---------------------------------------------------------------------------
# Manuscript table directory
# ---------------------------------------------------------------------------
# Mirrors get_manuscript_fig_dir(). The SI \inputs the tabular bodies from
# Overleaf_source/tables/, so a pipeline run must be able to write there --
# and must be able to be told not to, when checking a change without touching
# the manuscript.
get_manuscript_table_dir <- function() {
  tab_dir <- Sys.getenv("ACTIONSPACE_TABLE_DIR", unset = "")

  if (!nzchar(tab_dir)) {
    overleaf <- file.path("..", "Overleaf_source", "tables")
    tab_dir  <- if (dir.exists(overleaf)) {
      overleaf
    } else {
      file.path("results", "tables_manuscript")
    }
  }
