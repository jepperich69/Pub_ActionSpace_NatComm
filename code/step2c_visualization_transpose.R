...
p_sharp <- ggplot(diff_away, aes(x = t, y = d, fill = diff_density)) +
  ...
  labs(title = "Change in Action-Space Density Over Time by Age Group",
       subtitle = "Each row = one age group across time periods. Red = increased density, Blue = decreased density",
       x = "Time of Day (hours)", y = "Distance from Home (km)", fill = "Δ Density") +
  base_theme

# Figure 3. Canvas sized so base_size 11 lands near 7 pt once the figure is
# placed at \textwidth (6.69 in) in the manuscript. See R2 comment 2.6.
ggsave(file.path(out_dir, "kde_DIFFERENCE_by_cohort_SHARP.png"),
       p_sharp, width = 8.4, height = 5.6, dpi = 600, type = "cairo")
...
p_sharp_agg <- ggplot(diff_away, aes(x = t, y = d, fill = diff_density)) +
  ...
  labs(title = "Change in Action-Space Density — ENHANCED CONTRAST",
       subtitle = "Each row = one age group. 10th–90th percentile scaling, 600 DPI",
       x = "Time of Day (hours)", y = "Distance from Home (km)", fill = "Δ Density") +
  base_theme
...
p_publication <- ggplot(diff_away, aes(x = t, y = d, fill = diff_density)) +
  ...
  facet_grid(AgeGroup ~ year_bin) +   # *** TRANSPOSED ***
  labs(title = "Change in Action-Space Density Over Time by Age Group",
       subtitle = "Publication quality (1200 DPI). Each row = one age group. Red = increased, Blue = decreased",
       x = "Time of Day (hours)", y = "Distance from Home (km)", fill = "Δ Density") +
