...
cat("Computing action-space metrics (Eq. 6 and Eq. 7)...\n\n")

# Eq. (7) and the other per-cell metrics are defined once, in eq7_metrics.R.
# This file and step4_integral_R2.R previously held identical private copies,
# and the second overwrites the first's Figure 6b and Figure S3, so a change
# made in one of them silently did nothing.
source("code/eq7_metrics.R")
...
yearbin_metrics <- compute_cell_metrics(yearbin_densities_did, HOME_CUTOFF)
...
metric_changes <- yearbin_metrics %>%
  ...
  mutate(
    ...
    pct_change_fraction_away = 100 * (fraction_away - fraction_away_baseline) / fraction_away_baseline
  )
...
p_distance_trend <- yearbin_metrics %>%
  ...
  labs(title = "Mean Distance When Away Over Time",
       subtitle = "Dashed lines = arithmetic mean across periods",
       x = "Survey Period", 
...
# Supplementary Figure S3, placed at \textwidth (6.5 in) in the SI. This script
# runs after step4_integral.R and overwrites its copy, so the two must agree.
ggsave(file.path(output_dir, "mean_distance_trend_TRIPWEIGHTED.png"),
       p_distance_trend, width = 9.7, height = 6.8, dpi = 300)
...
p_trajectory <- yearbin_metrics %>%
  ...
  labs(title = "Action-Space Trajectory",
       subtitle = "Diamond = arithmetic mean baseline,\npath shows evolution",
       x = "Fraction of Day Away from Home",
       y = "Mean Distance When Away (km)",
       color = "Age Group", shape = "Period") +
  theme_minimal(base_size = 13) +
  # Legend at the bottom: at 4.8 in a right-hand legend leaves too little width
  # and the title runs off the canvas.
  theme(legend.position = "bottom", legend.box = "vertical",
        legend.text = element_text(size = 9), legend.title = element_text(size = 9),
        legend.key.size = unit(9, "pt"), legend.margin = margin(t = 0, b = 0)) +
  guides(color = guide_legend(nrow = 2, byrow = TRUE, order = 1),
         shape = guide_legend(nrow = 2, byrow = TRUE, order = 2))

# Figure 6b. It sits in a 0.48\textwidth subfigure (3.21 in), so the canvas has
# to be small for base_size 13 to reach 7 pt. Figure 6a is sized to match. This
# script runs after step4_integral.R and overwrites its copy.
ggsave(file.path(output_dir, "action_space_trajectory_TRIPWEIGHTED.png"),
       p_trajectory, width = 4.8, height = 4.6, dpi = 400)
...
p_decomposition <- metric_changes %>%
  ...
  labs(title = "Mobility Decomposition",
...
# Plot 4: Heatmap (UPDATED to 2-panel layout for R2)
library(patchwork)

p_heatmap_dist <- metric_changes %>%
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red",
                       ...
                       limits = c(-35, 35)) +
  labs(title = "A) Spatial Reach: % Change in Mean Distance When Away",
       x = "Survey Period", y = "Age Group") +
  theme_minimal(base_size = 14) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid = element_blank())

p_heatmap_time <- metric_changes %>%
  ggplot(aes(x = Period, y = AgeGroup, fill = pct_change_fraction_away)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%+.1f%%", pct_change_fraction_away)), 
            size = 4, fontface = "bold") +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red",
                       midpoint = 0, name = "% Change",
                       limits = c(-35, 35)) +
  labs(title = "B) Temporal Presence: % Change in Fraction of Day Away from Home",
       x = "Survey Period", y = "Age Group") +
  theme_minimal(base_size = 14) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid = element_blank())

p_heatmap_combined <- p_heatmap_dist / p_heatmap_time + 
...
cat("\n=== Summary: mean distance when away by age group (Eq. 7) ===\n")
print(summary_table)
cat("\n")
