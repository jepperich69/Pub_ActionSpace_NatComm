...
# ggrepel pushes labels away from points and from each other, but it knows
# nothing about the arrows, which is why the 10-17 label kept landing on its
# own shaft. Sampling each arrow into unlabelled points makes the shafts
# obstacles the repel layer can see. These rows carry label = "" so they draw
# nothing themselves.
arrow_obstacles <- drift_all %>%
  select(scenario_label, AgeGroup, x_base, y_base, x_end, y_end) %>%
  rowwise() %>%
  reframe(
    scenario_label = scenario_label,
    AgeGroup       = AgeGroup,
    x_end          = seq(x_base, x_end, length.out = 8),
    y_end          = seq(y_base, y_end, length.out = 8),
    AgeLabel       = ""
  ) %>%
  ungroup()

label_data <- drift_all %>%
  mutate(AgeLabel = as.character(AgeGroup)) %>%
  select(scenario_label, AgeGroup, x_end, y_end, AgeLabel) %>%
  bind_rows(arrow_obstacles)
...
p_panels <- ggplot(drift_all, aes(color = AgeGroup)) +
  ...
  #
  # Repelled rather than nudged by a fixed offset: the 10-17 and 18-30
  # endpoints sit close together in the urban-restricted panels, and a
  # constant nudge_y dropped the 10-17 label on top of the 18-30 arrow.
  # seed is set because the layout is otherwise random, and the figure is
  # checked byte for byte against results/figure_checksums.csv.
  ggrepel::geom_text_repel(
    data = label_data,
    aes(x = x_end, y = y_end, label = AgeLabel),
    size = 3, fontface = "bold", show.legend = FALSE,
    color = "gray30", seed = 42, inherit.aes = FALSE,
    box.padding = 0.45, point.padding = 0.25,
    min.segment.length = 0.25, segment.color = "gray70",
    segment.linewidth = 0.3, max.overlaps = Inf
  ) +
  ...
  labs(
    ...
    # No subtitle. The arrow convention and the shared axis scaling are stated
    # in the caption of Supplementary Fig. S4, which is where they belong.
...
# Supplementary Figure S4, placed at \textwidth (6.5 in) in the SI.
ggsave(file.path(OUT_DIR, "urban_drift_comparison.png"),
       p_panels, width = 8.9, height = 5.9, dpi = 300)
