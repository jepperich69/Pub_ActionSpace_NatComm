...
# Every kernel set the paper rests on, including the four sweep scenarios that
# only steps 9 and 9b read. The restricted stage rebuilds all of them; the
# public stage runs steps 2b-6 on the seven above.
KERNEL_SCENARIOS <- c(SCENARIOS, "bw3", "bw7", "ht05", "ht20")
...
if (manifest_only) {
  ...
  # One process per scenario. step2 guards H_T/H_D with `if (!exists(...))`,
  # so two scenarios in one session silently share the first bandwidth.
  for (s in KERNEL_SCENARIOS) {
    run_step("step2_run_scenario.R", s,
             label = paste0("step2 - kernels: ", s))
  }
} else {
  ...
  cat("all eleven kernel sets in data/kernels/ come from step 2 and are shipped.\n")
}
...
if (!manifest_only) {
  ...
  run_step("fig1_activity_graph.R",      label = "fig1 - Figure 1, illustrative action-space trajectory")
  ...
  run_step("step9_bandwidth_sensitivity.R", label = "step9 - Supplementary Figure S6, Table S6")
  run_step("step9b_temporal_bandwidth.R",   label = "step9b - Supplementary Table S7")
  ...
  run_step("build_si_tables.R",          label = "tables - Supplementary Tables S4-S8 bodies")
...
# Tables are addressed the same way as figures, through the manuscript
# directory rather than through results/. That is what makes a redirected run
# (ACTIONSPACE_FIG_DIR / ACTIONSPACE_TABLE_DIR) a real comparison: the rerun
# writes to scratch and the recorded reference still points at the manuscript.
# Addressed through results/ instead, a table would be compared against the
# very file the rerun had just overwritten.
tab_dir <- get_manuscript_table_dir()

# owner = NA marks an asset no script in this repository produces.
manifest <- rbind(
  # ---- main text figures ----
  data.frame(item = "Figure 1",  file = file.path(fig_dir, "activity_graph_v2_small.png"),           owner = "fig1_activity_graph.R"),
  ...
  # ---- tables ----
  data.frame(item = "Table 1",   file = file.path(tab_dir, "table_main1_descriptive.tex"),           owner = "RESTRICTED"),
  data.frame(item = "Table S1",  file = file.path(tab_dir, "table_s1_descriptive.tex"),              owner = "RESTRICTED"),
  data.frame(item = "Table S4",  file = file.path(tab_dir, "table_s4_covid_departure.tex"),          owner = "step11_covid_departure.R"),
  data.frame(item = "Table S5",  file = file.path(tab_dir, "table_s5_path_complexity.tex"),          owner = "build_si_tables.R"),
  data.frame(item = "Table S6",  file = file.path(tab_dir, "table_s6_bw_sensitivity.tex"),           owner = "build_si_tables.R"),
  data.frame(item = "Table S7",  file = file.path(tab_dir, "table_s7_ht_sensitivity.tex"),           owner = "build_si_tables.R"),
  data.frame(item = "Table S8",  file = file.path(tab_dir, "table_s8_marginals.tex"),                owner = "build_si_tables.R")
)
...
# currently are not.
#
# RESTRICTED does not excuse a missing file. Table 1 had no file at all and
# still reported clean for a full revision cycle, because this test used to
# short-circuit on the owner before it looked at presence.
manifest$status <- ifelse(
  manifest$owner %in% "RESTRICTED",
    ifelse(manifest$present, "RESTRICTED", "MISSING-RESTRICTED"),
...
content_diff <- function(a, b) {
  ...
  # The reference and the target must be two different files. figure_checksums
  # records ref_path as the figure's location in Overleaf_source/figures, which
  # is also where the pipeline writes by default -- so a plain rerun overwrote
  # the reference and then compared the new figure against itself, scored 0%
  # changed pixels, and reported RENDER-ONLY. Every content change passed
  # silently; the MD5 was the only thing still doing any work. Refuse the
  # comparison instead of inventing a verdict.
  if (normalizePath(a, winslash = "/", mustWork = FALSE) ==
      normalizePath(b, winslash = "/", mustWork = FALSE)) return(NA_real_)
...
if (file.exists(REF_FILE)) {
  ...
  for (i in seq_len(nrow(manifest))) {
    ...
    same_path <- !is.na(ref$ref_path[j]) &&
      normalizePath(ref$ref_path[j], winslash = "/", mustWork = FALSE) ==
      normalizePath(manifest$file[i], winslash = "/", mustWork = FALSE)

    manifest$px_diff[i] <- frac
    manifest$bitwise[i] <- if (same_path)            "REF-OVERWRITTEN"
                           else if (is.na(frac))     "DIFFERS-?"
...
if (n_missing > 0) {
  ...
  cat("   Figure S7, Table 1 and Table S1 are expected to be missing unless the\n")
...
n_overwr  <- sum(manifest$bitwise == "REF-OVERWRITTEN")
n_differs <- n_content + n_unknown + n_overwr
...
if (file.exists(REF_FILE)) {
  ...
  if (n_overwr > 0) {
    cat("\n  ", n_overwr, "REF-OVERWRITTEN - NOT VERIFIED:\n")
    for (f in manifest$item[manifest$bitwise == "REF-OVERWRITTEN"]) cat("      -", f, "\n")
    cat("   The bytes changed, and the reference for these is the very file\n")
    cat("   this run overwrote, so nothing is left to compare against. This\n")
    cat("   was a REGENERATION run, not a verification run. To verify, send\n")
    cat("   the output elsewhere and compare against the manuscript:\n")
    cat("       ACTIONSPACE_FIG_DIR=<scratch> Rscript code/run_paper.R\n")
  }
