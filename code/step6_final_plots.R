...
# Endpoint displacement, measured on the same chain as the path length:
# first real period to last. The baseline-referenced drift magnitude used here
# previously mixed two different start points and inflated every tortuosity value.
chain_endpoints <- all_sequential %>%
  group_by(AgeGroup) %>%
  summarise(
    endpoint_displacement = sqrt((dplyr::last(x_end)  - dplyr::first(x_start))^2 +
                                 (dplyr::last(y_end)  - dplyr::first(y_start))^2),
    .groups = "drop"
  )

path_metrics <- path_metrics %>%
  left_join(chain_endpoints, by = "AgeGroup") %>%
  mutate(
