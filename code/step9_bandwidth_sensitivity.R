...
# Eq. (7) is defined once, in eq7_metrics.R. This script used to keep a
# private copy, which meant the bandwidth sensitivity could silently test a
# different estimator from the one the main figures use. The fixed-weight
# reference profile is rebuilt inside each bandwidth scenario, which is what
# the sensitivity question asks: does the PROCEDURE give a stable answer.
...
all_metrics <- lapply(names(bandwidths), function(bw_name) {
  ...
  metrics <- compute_cell_metrics(dat, HOME_CUTOFF) %>%
