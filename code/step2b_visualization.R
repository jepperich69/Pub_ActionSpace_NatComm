  ...
  if (length(periods) >= 2) {
    ...
    if (nrow(yearbin_away) > 0) {
      ...
      p_yearbin <- ggplot(yearbin_away, aes(x = t, y = d, fill = density)) +
        ...
        facet_grid(year_bin ~ AgeGroup) +
        # Explicit breaks: the default scale ran to a 25th hour, which the day
        # does not have.
      ...
      # Figure 2. Canvas sized so base_size 11 lands near 7 pt once the figure
      # is placed at \textwidth (6.69 in) in the manuscript. See R2 comment 2.6.
      ggsave(file.path(out_dir, "kde_YEARBIN_comparison_FIXED.png"), p_yearbin, width = 8.4, height = 4.8, dpi = 300)
