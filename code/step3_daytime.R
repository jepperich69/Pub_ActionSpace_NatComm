...
# Figure 4. Canvas sized so base_size 12 lands near 7 pt at \textwidth
# (6.69 in) in the manuscript. See R2 comment 2.6.
ggsave(file.path(output_dir, "temporal_profiles_all_ages.png"),
       p_all_ages, width = 9.2, height = 7.7, dpi = 300)
...
# Supplementary Figure S2, placed at \textwidth (6.5 in) in the SI.
ggsave(file.path(output_dir, "mean_time_shift.png"),
       p_mean_shift, width = 9.7, height = 5.8, dpi = 300)
cat("  ✓ mean_time_shift.png\n")

# Plot 3: Window shifts
#
# A least-squares trend line is drawn over each window's bars. The morning and
# evening series move in opposite directions across the study period and the
# bars alone make that hard to read; the lines are what carry the result. The
# x axis is numeric rather than discrete so the lines can sit at the dodged
# positions of the bars they belong to, instead of at the period centre.
DODGE_W <- 0.9
WINDOW_LEVELS <- c("Morning (6-12h)", "Midday (12-18h)", "Evening (18-24h)")

window_shift_data <- temporal_shifts %>%
...
window_periods <- sort(unique(as.character(window_shift_data$Period)))

window_shift_data <- window_shift_data %>%
  mutate(period_num = match(as.character(Period), window_periods),
         # Centre of the bar this point belongs to: three dodged bars per period.
         x_dodge = period_num +
           (as.integer(Window) - 2) * (DODGE_W / length(WINDOW_LEVELS)))

p_window_shifts <- window_shift_data %>%
  ggplot(aes(x = period_num, y = Shift, fill = Window)) +
  geom_col(position = position_dodge(width = DODGE_W)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  geom_smooth(aes(x = x_dodge, colour = Window), method = "lm", formula = y ~ x,
              se = FALSE, linewidth = 0.7, show.legend = FALSE) +
  facet_wrap(~ AgeGroup, ncol = 2) +
  scale_x_continuous(breaks = seq_along(window_periods), labels = window_periods) +
...
# Supplementary Figure S1, placed at \textwidth (6.5 in) in the SI.
ggsave(file.path(output_dir, "window_shifts.png"),
       p_window_shifts, width = 8.9, height = 7.4, dpi = 300)
